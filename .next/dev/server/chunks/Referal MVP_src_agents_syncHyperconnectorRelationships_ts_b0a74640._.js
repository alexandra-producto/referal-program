module.exports = [
"[project]/Referal MVP/src/agents/syncHyperconnectorRelationships.ts [app-route] (ecmascript)", ((__turbopack_context__) => {
"use strict";
 ...
 */ function dateRangesOverlap(start1, end1, start2, end2) {
    ...
    const monthsDiff = (overlapEnd.getFullYear() - overlapStart.getFullYear()) * 12 + (overlapEnd.getMonth() - overlapStart.getMonth());
 ...
 */ function findWorkOverlaps(hyperconnectorId, hciExperiences, candidateId, candidateExperiences) {
    ...
    for (const hciExp of hciExperiences){
        ...
        for (const candidateExp of candidateExperiences){
            ...
            // Check date overlap
            const { overlaps, overlapMonths } = dateRangesOverlap(hciExp.start_date, hciExp.end_date, candidateExp.start_date, candidateExp.end_date);
            ...
            const confidenceScore = overlapMonths >= 12 ? 100 : Math.max(10, Math.min(100, Math.round(overlapMonths / 12 * 100)));
 ...
 */ async function getCandidatesInHyperconnectorNetwork(hyperconnectorId) {
    // Try candidate_connections first (new table)
    const { data: connections } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("candidate_connections").select("candidate_id").eq("hyperconnector_id", hyperconnectorId);
    if (connections && connections.length > 0) {
        return connections.map((c)=>c.candidate_id);
    }
    // Fallback to hyperconnector_candidates
    const { data: hciCandidates } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("hyperconnector_candidates").select("candidate_id").eq("hyperconnector_id", hyperconnectorId);
    if (hciCandidates && hciCandidates.length > 0) {
        return hciCandidates.map((c)=>c.candidate_id);
    }
    return [];
}
/**
 ...
 */ async function getHyperconnectorCandidateId(hyperconnectorId) {
    // Check if hyperconnector has a candidate_id field or linked candidate
    const { data: hci } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("hyperconnectors").select("candidate_id").eq("id", hyperconnectorId).maybeSingle();
 ...
 */ async function relationshipExists(hyperconnectorId, candidateId, relationshipType = "worked_together") {
    // Check in candidate_connections first (new table with source field)
    const { data: connection, error: connError } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("candidate_connections").select("id").eq("hyperconnector_id", hyperconnectorId).eq("candidate_id", candidateId).eq("source", "experience_overlap").maybeSingle();
    ...
    const { data: hciCandidate, error: hciError } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("hyperconnector_candidates").select("id").eq("hyperconnector_id", hyperconnectorId).eq("candidate_id", candidateId).maybeSingle();
 ...
 */ async function createOrUpdateRelationship(hyperconnectorId, candidateId, overlap) {
    const now = new Date().toISOString();
    // 1. Insert/update in candidate_connections (new table with source and connection_strength)
    try {
        const { error: connError } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("candidate_connections").upsert({
    ...
    try {
        ...
        const { data: existing, error: checkError } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("hyperconnector_candidates").select("id").eq("hyperconnector_id", hyperconnectorId).eq("candidate_id", candidateId).maybeSingle();
        if (checkError && checkError.code !== "PGRST116") {
            console.warn("⚠️ Error checking existing hyperconnector_candidates:", checkError.message);
        }
        if (existing) {
            // Update existing record
            const { data: updated, error: updateError } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("hyperconnector_candidates").update({
                relationship_type: "worked_together",
            ...
            // Insert new record
            const { data: inserted, error: insertError } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("hyperconnector_candidates").insert({
                hyperconnector_id: hyperconnectorId,
                candidate_id: candidateId,
                relationship_type: "worked_together",
                relationship_source: "automatic_match"
...
async function syncHyperconnectorCandidateRelationshipsForCandidate(candidateId) {
    console.log(`🔄 Syncing relationships for candidate: ${candidateId}`);
    // Get candidate's experience
    const candidateExperiences = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$domain$2f$candidateExperience$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getExperienceForCandidate"])(candidateId);
    if (candidateExperiences.length === 0) {
        console.log(`   ⚠️ No experience found for candidate ${candidateId}`);
        return;
    }
    // Get all hyperconnectors
    const hyperconnectors = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$domain$2f$hyperconnectors$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getAllHyperconnectors"])();
    ...
    for (const hci of hyperconnectors){
        ...
        const hciExperiences = await (0, __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$domain$2f$candidateExperience$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["getExperienceForCandidate"])(hciCandidateId);
        ...
        if (overlap) {
            ...
            if (!exists) {
                ...
                console.log(`   ✅ Found overlap: ${hci.full_name} ↔ candidate ${candidateId} at ${overlap.companyName} (${overlap.overlapMonths.toFixed(1)} months)`);
            } else {
                console.log(`   ⏭️  Relationship already exists: ${hci.full_name} ↔ candidate ${candidateId}`);
            }
        }
    }
    console.log(`✅ Sync complete: ${relationshipsCreated.length} new relationships created`);
}
async function syncHyperconnectorCandidateRelationshipsForHyperconnector(hyperconnectorId) {
    ...
    const { data: connections, error: connError } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("candidate_connections").select("candidate_id").eq("hyperconnector_id", hyperconnectorId);
    if (connError) {
    ...
    const candidateIds = connections.map((c)=>c.candidate_id);
    console.log(`   📋 Found ${candidateIds.length} candidate(s) in network`);
    // Get candidates already in hyperconnector_candidates
    const { data: existingRelations, error: existingError } = await __TURBOPACK__imported__module__$5b$project$5d2f$Referal__MVP$2f$src$2f$db$2f$supabaseClient$2e$ts__$5b$app$2d$route$5d$__$28$ecmascript$29$__["supabase"].from("hyperconnector_candidates").select("candidate_id").eq("hyperconnector_id", hyperconnectorId);
    if (existingError) {
        console.warn(`   ⚠️ Error checking existing relationships: ${existingError.message}`);
    }
    const existingCandidateIds = new Set((existingRelations || []).map((r)=>r.candidate_id));
    // Filter to only candidates NOT in hyperconnector_candidates
    const candidatesToCheck = candidateIds.filter((candidateId)=>!existingCandidateIds.has(candidateId) && candidateId !== hciCandidateId);
        ...
        if (overlap) {
            ...
            try {
                ...
                if (insertError) {
                    ...
                    relationshipsCreated.push(overlap);
                    console.log(`   ✅ Created relationship: candidate ${candidateId} at ${overlap.companyName} (${overlap.overlapMonths.toFixed(1)} months)`);
                }
            } catch (error) {
...
//# sourceMappingURL=Referal%20MVP_src_agents_syncHyperconnectorRelationships_ts_b0a74640._.js.map