...
/**
 * POST /api/recommend/submit?token=xxx
 * Workaround temporal para rutas dinámicas en Vercel
 * Envía una recomendación usando un token
 */
export async function POST(request: NextRequest) {
  ...
  try {
    console.log("📝 POST /api/recommend/submit - Token:", token.substring(0, 30) + "...");
    ...
    console.error("❌ Error en POST /api/recommend/submit:", error);
    return NextResponse.json(
      {
